  ...
  Widget build(BuildContext context) {
    ...
    Widget renderContainer = Container(
      ...
      child: Row(
        ...
        children: [
          if (icon != null) ...[
            buildIcon(icon: icon ?? const Icon(Icons.description), color: textStyle?.color),
            const SizedBox(width: 4),
          ],
          text,
          if (rightIcon != null) ...[
            const SizedBox(width: 4),
            buildIcon(icon: rightIcon ?? const Icon(Icons.description), color: textStyle?.color),
          ],
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    Widget renderContainer = Container(
      ...
      child: Row(
        ...
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          buildIcon(icon: icon ?? const Icon(Icons.description)),
        ],
      ),
    );
    if (onTap != null) {
